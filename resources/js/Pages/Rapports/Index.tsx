...
function SimpleBar({ data, dataKey, nameKey, color }: any) { if (!data?.length) return <EmptyState />; return <ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={1}><BarChart data={data}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey={nameKey} /><YAxis /><Tooltip /><Bar dataKey={dataKey} fill={color} radius={[6,6,0,0]} /></BarChart></ResponsiveContainer>; }
function SimpleLine({ data, xKey, yKey, color }: any) { if (!data?.length) return <EmptyState />; return <ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={1}><LineChart data={data}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey={xKey} /><YAxis /><Tooltip /><Line type="monotone" dataKey={yKey} stroke={color} strokeWidth={2} /></LineChart></ResponsiveContainer>; }
function SimplePie({ data, nameKey, valueKey }: any) { if (!data?.length) return <EmptyState />; const colors = ['#0B63CE', '#EF4444', '#16A34A']; return <ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={1}><PieChart><Pie data={data} dataKey={valueKey} nameKey={nameKey} outerRadius={90} label>{data.map((_:any, i:number) => <Cell key={i} fill={colors[i % colors.length]} />)}</Pie></PieChart></ResponsiveContainer>; }
